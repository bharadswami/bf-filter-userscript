...
(function () {
  ...
  function loadSettings() {
    return {
      ...
      minCharThreshold: GM_getValue(
        "minCharThreshold",
        DEFAULTS.minCharThreshold,
      ),
      keywordFilterEnabled: GM_getValue(
        "keywordFilterEnabled",
        DEFAULTS.keywordFilterEnabled,
      ),
      lengthFilterEnabled: GM_getValue(
        "lengthFilterEnabled",
  ...
  GM_registerMenuCommand("Set min character threshold", () => {
    const val = prompt(
      "Minimum characters:",
      String(settings.minCharThreshold),
    );
    if (val === null || !Number.isInteger(Number(val)) || Number(val) < 0)
      return;
...
[{"id": 1, "low": true, "reason": "brief reason"}, ...]`;
  ...
  function classifyComment(commentEl) {
    ...
    // Length filter
    if (
      settings.lengthFilterEnabled &&
      settings.minCharThreshold > 0 &&
      text.length < settings.minCharThreshold
    ) {
      return "length";
    }
    ...
    if (!bar) {
      ...
      bar.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-bf-filtered]").forEach((el) => {
          ...
          if (revealed) {
            li.style.display =
              li.getAttribute("data-bf-original-display") || "";
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    const hasNewComments = mutations.some((m) =>
      Array.from(m.addedNodes).some(
        (n) =>
          n.nodeType === 1 &&
          (n.matches?.(COMMENT_SEL) || n.querySelector?.(COMMENT_SEL)),
      ),
    );
